...
// ─────────────────────────────────────────────
// STATIC DATA – NGOs, Training, Pending Loans
// ─────────────────────────────────────────────
const NGOS = [
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,rgba(216,67,21,.06),rgba(255,167,38,.06))", fontFamily: "'Poppins', sans-serif" }}>
      <div style={{ background: "#fff", padding: 40, borderRadius: 16, boxShadow: "0 8px 32px rgba(45,27,16,.15)", width: "100%", maxWidth: 440, border: "1px solid #d4c5ad" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 24 }}>
          {[{ key: "member", icon: "👤", label: "Member" }, { key: "leader", icon: "⭐", label: "SHG Leader" }, { key: "superadmin", icon: "🛡️", label: "Super Admin" }].map((r) => (
...
// ─────────────────────────────────────────────
// PAGE: SUPER ADMIN – LOAN SANCTIONS
// ─────────────────────────────────────────────
function SuperAdminPage() {
  ...
  function approveLoan() {
    ...
    setToast(`Loan ${modal.id} sanctioned — ₹${amt.toLocaleString("en-IN")}`);
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:4 }}>
        <h1 style={{ fontSize:24, fontWeight:700, color:"#2d1b10" }}>Super Admin – Loan Sanctions</h1>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
        <StatCard icon="📋" value={loans.length}   label="Total Applications" />
        <StatCard icon="⏳" value={awaitCount}     label="Awaiting Approval"  gradientColor="#f57c00" />
        <StatCard icon="✓"  value={approvedCnt}    label="Approved"           gradientColor="#2e7d32" />
        <StatCard icon="🏦" value={bankPending}    label="Bank Pending"       gradientColor="#8b7355" />
      </div>
      ...
      {modal && (
        <div style={{
          ...
          <div style={{
            ...
            {/* close */}
            <button onClick={closeModal} style={{ position:"absolute", top:16, right:18, background:"none", border:"none", fontSize:22, color:"#8b7355", cursor:"pointer" }}>×</button>
            ...
            <div style={{ marginBottom:18 }}>
              ...
              <span style={{ fontSize:14, color:"#5d4e37" }}>{fmt(modal.requestedAmt)} — {modal.purpose}</span>
            </div>

            {/* amount input */}
            <div style={{ marginBottom:24 }}>
              <label style={{ fontSize:11, fontWeight:600, color:"#8b7355", textTransform:"uppercase", letterSpacing:0.5, display:"block", marginBottom:6 }}>Loan Amount to Sanction (₹)</label>
              <input
                type="number"
                value={inputAmt}
